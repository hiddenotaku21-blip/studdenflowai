...
export const chatWithSensei = async (
    ...
    // gemini-3-pro-preview for SMART responses
    const modelName = useLite ? 'gemini-2.5-flash-lite' : 'gemini-3-pro-preview';

    const chat = aiClient.chats.create({
        model: modelName,
        history: history,
        config: {
            systemInstruction: "You are StudenFlow-chan. Keep responses concise, helpful, and anime-styled. If using Lite mode, be very brief.",
        }
    });
